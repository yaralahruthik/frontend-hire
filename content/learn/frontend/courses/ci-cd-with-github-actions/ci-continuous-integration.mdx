...
<Callout type="info" title="References: I know this because I searched the internet!">

- [What type of constants are allowed to be exported when using Vite](https://github.com/ArnaudBarre/eslint-plugin-react-refresh/issues/84)

- [Components are exporting constants and functions causing
  Fast Refresh lint issue](https://github.com/shadcn-ui/ui/issues/7736)

- [ESLint: Disable rules using configuration
  files](https://eslint.org/docs/latest/use/configure/rules#using-configuration-files-1)
...
<Callout type="warn" title="Always test for failing builds">
  I repeat again! It is quite easy to assume the action works as expected but
  testing for failing builds is a better validation. So, intentionally break a
  test and check for correct failure.
</Callout>

At this point, our code should match the code in the branch [`1-ci-setup`](https://github.com/yaralahruthik/fh-ci-cd-todo-app/tree/1-ci-setup).
